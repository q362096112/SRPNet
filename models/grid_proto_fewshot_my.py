...
class Block(nn.Module):
    ...
    def forward(self, x, x_feat):
        feat_temp = torch.cat([x, x_feat], dim=1)
        feat_temp = self.conv(feat_temp)
        out = x + feat_temp
        return out
...
class FewShotSeg(nn.Module):
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs, isval, val_wsize, show_viz = False):
        ...
        for epi in range(1): # batch dimension, fixed to 1
            ...
            bg = pred[:, 0].reshape(1, 1, pred.shape[-2], pred.shape[-1])
            fg = pred[:, 1].reshape(1, 1, pred.shape[-2], pred.shape[-1])
            bg_feat = self.conv1(bg)
            fg_feat = self.conv1(fg)


            bg_pred = self.block1(bg, fg_feat)
            fg_pred = self.block2(fg, bg_feat)
    ...
    def alignLoss(self, qry_fts, pred, supp_fts, fore_mask, back_mask):
        ...
        for way in range(n_ways):
            ...
            for shot in range(n_shots):
                ...
                bg = supp_pred[:, 0].reshape(1, 1, supp_pred.shape[-2], supp_pred.shape[-1])
                fg = supp_pred[:, 1].reshape(1, 1, supp_pred.shape[-2], supp_pred.shape[-1])
                bg_feat = self.conv1(bg)
                fg_feat = self.conv1(fg)

                bg_pred = self.block1(bg, fg_feat)
                fg_pred = self.block2(fg, bg_feat)
                supp_pred = torch.cat([bg_pred, fg_pred], dim=1)
