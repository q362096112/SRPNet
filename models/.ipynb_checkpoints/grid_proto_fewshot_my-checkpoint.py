...
class Block(nn.Module):
    def __init__(self, dim=256, num_heads=8, qkv_scale=None):
        super().__init__()
        self.scale = qkv_scale or num_heads ** -0.5
    ...
    def forward(self, qry, supp, supp_mask):
        supp = supp * supp_mask
        qry = qry.flatten(2).transpose(1, 2)
        supp = supp.flatten(2).transpose(1, 2)

        B, N, C = qry.shape

        q_q, k_q, v_q = self.qkv(qry).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q_s, k_s, v_s = self.qkv(supp).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)

        attn = (q_q @ k_s.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        qry_feat = (attn @ v_s).transpose(1, 2).reshape(B, C, 32, 32)
        qry_feat = self.ada(qry_feat)
        qry_feat = qry_feat.transpose(1, 3).contiguous()
        qry_feat = self.linear2(self.relu(self.linear1(qry_feat)))
        qry_feat = qry_feat.transpose(1, 3).contiguous()

        attn = (q_s @ k_q.transpose(-2, -1)) * self.scale
        attn = attn.softmax(-1)
        supp_feat = (attn @ v_q).transpose(1, 2).reshape(B, C, 32, 32)
        supp_feat = self.ada(supp_feat)
        supp_feat = supp_feat.transpose(1, 3).contiguous()
        supp_feat = self.linear2(self.relu(self.linear1(supp_feat)))
        supp_feat = supp_feat.transpose(1, 3).contiguous()

        qry_feat = self.sigmoid(qry_feat)
        supp_feat = self.sigmoid(supp_feat)
        temp = qry_feat * supp_feat
        return temp
...
class FewShotSeg(nn.Module):
    ...
    def get_cls(self):
        ...
        if self.config['cls_name'] == 'grid_proto':
            self.cls_unit = MultiProtoAsConv(proto_grid=[proto_hw, proto_hw], feature_hw=self.config[
                "feature_hw"])  # when treating it as ordinary prototype
        else:
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs, isval, val_wsize, show_viz=False):
        ...
        supp_fts = supp_fts.reshape(1, 256, 32, 32)

        qry_fts = qry_fts.reshape(1, 256, 32, 32)

        temp_fore = fore_mask.reshape(1, 1, 256, 256)
        temp_fore = F.interpolate(temp_fore, size=fts_size, mode='bilinear')

        temp = self.block(qry_fts, supp_fts, temp_fore)

        supp_fts = (supp_fts * temp).view(
            n_ways, n_shots, sup_bsize, -1, *fts_size)
        qry_fts = (qry_fts * temp).view(
        ...
        for epi in range(1):  # batch dimension, fixed to 1
            ...
            res_fg_msk = torch.stack(
                [F.interpolate(fore_mask_w, size=fts_size, mode='bilinear') for fore_mask_w in fore_mask],
            ...
            for way, _msk in enumerate(res_fg_msk):
                _raw_score, _, aux_attr = self.cls_unit(qry_fts, supp_fts, _msk.unsqueeze(0),
                                                        mode=FG_PROT_MODE if F.avg_pool2d(_msk,
                                                                                          4).max() >= FG_THRESH and FG_PROT_MODE != 'mask' else 'mask',
        ...
        for way in range(n_ways):
            ...
            for shot in range(n_shots):
                ...
                _raw_score_fg, _, _ = self.cls_unit(qry=img_fts, sup_x=qry_fts, sup_y=qry_pred_fg_msk.unsqueeze(-3),
                                                    mode=FG_PROT_MODE if F.avg_pool2d(qry_pred_fg_msk,
                                                                                      4).max() >= FG_THRESH and FG_PROT_MODE != 'mask' else 'mask',
                                                    thresh=FG_THRESH)
